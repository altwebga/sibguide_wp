<?php
/**
 * CloudPayments Account widget template.
 *
 * @since 1.0
 */
if ( ! defined('ABSPATH') ) {
...
<div class="ts-vendor-settings hidden">
	<div v-if="screen === 'main'" class="ts-panel">
		<div class="ac-head">
		   <?= \Voxel\get_icon_markup( $this->get_settings_for_display('ts_connect_ico') ) ?: \Voxel\svg( 'cloudpayments.svg' ) ?>
		   <b><?= _x( 'CloudPayments Connect', 'cloudpayments vendor', 'voxel' ) ?></b>
		</div>
		<div class="ac-body">
			<?php if ( \Voxel\current_user()->has_cap('administrator') && apply_filters( 'voxel/cloudpayments_connect/enable_onboarding_for_admins', false ) !== true ) : ?>
				<p><?= _x( 'CloudPayments vendor onboarding is not necessary for admin accounts.', 'cloudpayments vendor', 'voxel' ) ?></p>
			<?php else: ?>
				<?php if ( $account->charges_enabled ): ?>
					<p><?= _x( 'Your account is ready to accept payments.', 'cloudpayments vendor', 'voxel' ) ?></p>
				<?php elseif ( $account->details_submitted ): ?>
					<p><?= _x( 'Your account is pending verification.', 'cloudpayments vendor', 'voxel' ) ?></p>
				<?php else: ?>
					<p><?= _x( 'Setup your CloudPayments vendor account in order to accept payments.', 'cloudpayments vendor', 'voxel' ) ?></p>
				<?php endif ?>
				<div class="ac-bottom">
						...
						<?php if ( ! $account->exists ): ?>
							<li>
								<a href="<?= esc_url( $onboard_link ) ?>" class="ts-btn ts-btn-1 ts-btn-large">
									...
									<?= _x( 'Start setup', 'cloudpayments vendor', 'voxel' ) ?>
								</a>
						...
						<?php elseif ( ! $account->details_submitted ): ?>
							<li>
								<a href="<?= esc_url( $onboard_link ) ?>" class="ts-btn ts-btn-1 ts-btn-large">
									...
									<?= _x( 'Complete onboarding', 'cloudpayments vendor', 'voxel' ) ?>
								</a>
							</li>
							<?php if ( \Voxel\get( 'product_settings.multivendor.shipping.responsibility' ) === 'vendor' ): ?>
								<li>
									<a href="#" class="ts-btn ts-btn-1 ts-btn-large" @click.prevent="screen = 'shipping'">
										...
										<?= _x( 'Configure Shipping', 'cloudpayments vendor', 'voxel' ) ?>
									</a>
								</li>
							<?php endif ?>
						<?php else: ?>
							...
							<li>
								<a href="<?= esc_url( $dashboard_link ) ?>" target="_blank" class="ts-btn ts-btn-1 ts-btn-large">
									 <?= \Voxel\get_icon_markup( $this->get_settings_for_display('ts_cloudpayments_ico') ) ?: \Voxel\svg( 'cloudpayments.svg' ) ?>
									<?= _x( 'Vendor dashboard', 'cloudpayments vendor', 'voxel' ) ?>
								</a>
							</li>
							<li>
								<a href="<?= esc_url( $onboard_link ) ?>" class="ts-btn ts-btn-1 ts-btn-large">
									 <?= \Voxel\get_icon_markup( $this->get_settings_for_display('ts_update_ico') ) ?: \Voxel\svg( 'menu.svg' ) ?>
									<?= _x( 'Update information', 'cloudpayments vendor', 'voxel' ) ?>
								</a>
							</li>
							<?php if ( \Voxel\get( 'product_settings.multivendor.shipping.responsibility' ) === 'vendor' ): ?>
								<li>
									<a href="#" class="ts-btn ts-btn-1 ts-btn-large" @click.prevent="screen = 'shipping'">
										 <?= \Voxel\get_icon_markup( $this->get_settings_for_display('ts_shipping_ico') ) ?: \Voxel\svg( 'fast-delivery.svg' ) ?>
										<?= _x( 'Shipping', 'cloudpayments vendor', 'voxel' ) ?>
									</a>
								</li>
							<?php endif ?>
					...
					<div class="create-form-step form-field-grid">
						<div class="ts-form-group  vx-1-2">
							<a @click.prevent="screen = 'main'" href="#" class="ts-btn ts-btn-1 ts-btn-large">
								...
								<?= _x( 'Go back', 'cloudpayments vendor shipping', 'voxel' ) ?>
							</a>
						</div>
						<div class="ts-form-group vx-1-2">
							<a @click.prevent="saveShipping" href="#" class="ts-btn ts-btn-2 ts-btn-large" :class="{'vx-disabled': savingShipping}">
								...
								<?= _x( 'Save changes', 'cloudpayments vendor shipping', 'voxel' ) ?>
							</a>
						</div>
						<div class="ts-form-group ui-heading-field field-key-ui-heading"><label><?= _x( 'Configure Shipping', 'cloudpayments vendor', 'voxel' ) ?></label></div>
						<div class="ts-form-group">
							<label><?= _x( 'Shipping zones', 'cloudpayments vendor', 'voxel' ) ?></label>
								...
								<template v-for="zone, zoneIndex in config.zones">
									<div class="ts-field-repeater" :class="{collapsed: activeZone !== zone}">
										<div class="ts-repeater-head" @click.prevent="activeZone = activeZone === zone ? null : zone">
											...
											<label>{{ zone.label || <?= wp_json_encode( _x( '(untitled)', 'cloudpayments vendor shipping', 'voxel' ) ) ?> }}</label>
										...
										<div v-if="activeZone === zone" class="medium form-field-grid">
											<div class="ts-form-group vx-1-2">
												<label><?= _x( 'Label', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
											...
											<form-group :popup-key="'countries'" class="vx-1-2" :ref="'shipping:'+zone.key" @clear="zone.countries = []" @save="$refs['shipping:'+zone.key]?.[0]?.blur()">
												<template #trigger>
													<label>
														<?= _x( 'Countries', 'cloudpayments vendor shipping', 'voxel' ) ?>
													</label>
													<div class="ts-filter ts-popup-target" :class="{'ts-filled': zone.countries.length}" @mousedown="$root.activePopup = 'countries'">
														<div class="ts-filter-text">
															<span>{{ getShippingCountriesLabel(zone) || <?= wp_json_encode( _x( 'Select one or more countries', 'cloudpayments vendor shipping', 'voxel' ) ) ?> }}</span>
														</div>
													</div>
												</template>
												<template #popup>
													<div class="ts-sticky-top uib b-bottom">
														<div class="ts-input-icon flexify">
															...
															<input v-model="zone._country_search" ref="searchInput" type="text" placeholder="<?= esc_attr( _x( 'Search countries', 'cloudpayments vendor shipping', 'voxel' ) ) ?>" class="autofocus">
														</div>
													</div>
														...
														<ul class="simplify-ul ts-term-dropdown-list min-scroll">
															<li v-if="!zone._country_search?.trim().length" @click.prevent="selectAllShippingCountries(zone)">
																<a href="#" class="flexify">
																	...
																	<span>
																		<?= _x( 'All countries', 'cloudpayments vendor shipping', 'voxel' ) ?>
																	</span>
																</a>
															</li>
											...
											<div class="ts-form-group vx-1-1">
												<label><?= _x( 'Shipping rates', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
													...
													<template v-for="rate, rateIndex in zone.rates">
														<div class="ts-field-repeater" :class="{collapsed: zone._active_rate !== rate}">
															<div class="ts-repeater-head" @click.prevent="zone._active_rate = zone._active_rate === rate ? null : rate">
																<?= \Voxel\get_icon_markup( $this->get_settings_for_display('ts_shipping_ico') ) ?: \Voxel\svg( 'fast-delivery.svg' ) ?>
																<label>{{ rate.label || <?= wp_json_encode( _x( '(untitled)', 'cloudpayments vendor shipping', 'voxel' ) ) ?> }}</label>
																<div class="ts-repeater-controller">
															...
															<div v-if="zone._active_rate === rate" class="medium form-field-grid">
																...
																<div class="ts-form-group vx-1-2">
																	<label><?= _x( 'Type', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
																	<div class="ts-filter">
																		<select v-model="rate.type">
																			<option value="free_shipping"><?= _x( 'Free shipping', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																			<option value="fixed_rate"><?= _x( 'Fixed rate', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																		</select>
																	</div>
																</div>
																...
																<template v-if="rate.type === 'free_shipping'">
																	<div class="ts-form-group vx-1-1">
																		<label><?= _x( 'Free shipping requires', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
																		<div class="ts-filter">
																			<select v-model="rate.free_shipping.requirements">
																				<option value="none"><?= _x( 'No requirement', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																				<option value="minimum_order_amount"><?= _x( 'Minimum order amount', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																			</select>
																		</div>
																	</div>
																	...
																	<template v-if="rate.free_shipping.requirements === 'minimum_order_amount'">
																		<div class="ts-form-group vx-1-1">
																			<label><?= _x( 'Min. order amount', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
																	...
																	<div class="ts-form-group vx-1-1">
																		<label>
																			...
																			<?= _x( 'Add delivery estimate?', 'cloudpayments vendor shipping', 'voxel' ) ?>
																		</label>

																		<template v-if="rate.free_shipping.delivery_estimate.enabled">
																			<div class="medium form-field-grid">
																				<div class="ts-form-group vx-1-2">
																					<label><?= _x( 'Between', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
																					<input v-model="rate.free_shipping.delivery_estimate.minimum.value" type="number" class="ts-filter">
																				</div>
																				<div class="ts-form-group vx-1-2">
																					<label><?= _x( 'Period', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
																					<div class="ts-filter">
																						<select v-model="rate.free_shipping.delivery_estimate.minimum.unit">
																							<option value="hour"><?= _x( 'Hour(s)', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																							<option value="day"><?= _x( 'Day(s)', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																							<option value="business_day"><?= _x( 'Business day(s)', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																							<option value="week"><?= _x( 'Week(s)', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																							<option value="month"><?= _x( 'Month(s)', 'cloudpayments vendor shipping', 'voxel' ) ?></option>
																						</select>
																					</div>
																				</div>
																				<div class="ts-form-group vx-1-2">
																					<label><?= _x( 'And', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
																...
																<template v-else-if="rate.type === 'fixed_rate'">
																	<div class="ts-form-group vx-1-1">
																		<label><?= _x( 'Price per unit', 'cloudpayments vendor shipping', 'voxel' ) ?></label>
																		<div class="input-container">
																	...
																	<div v-if="Object.keys(config.shipping_classes).length" class="ts-form-group vx-1-1">
																		...
																		<div class="medium form-field-grid">
																			<div class="ts-form-group vx-1-1 ui-heading-field">
																				<label><?= _x( 'Price by class', 'cloudpayments vendor shipping', 'voxel' ) ?>
												...
												<a href="#" @click.prevent="addShippingRate(zone)" class="ts-repeater-add ts-btn ts-btn-4 form-btn">
													...
													<?= _x( 'Add shipping rate', 'cloudpayments vendor shipping', 'voxel' ) ?>
												</a>
											</div>
										</div>
									</div>
							...
							<a href="#" class="ts-repeater-add ts-btn ts-btn-4 form-btn" @click.prevent="config.zones.push( activeZone = {
								...
								<?= \Voxel\get_icon_markup( $this->get_settings_for_display('ts_add_icon') ) ?: \Voxel\svg( 'plus.svg' ) ?>
								<?= _x( 'Add shipping zone', 'cloudpayments vendor shipping', 'voxel' ) ?>
							</a>
						</div>
						<!-- <div class="vx-1-1">
