 ...
 *
 * On the other hand <a href="https://cloudpayments.com/docs/api#external_accounts">External Accounts</a> are transfer
 * destinations on <code>Account</code> objects for <a href="https://cloudpayments.com/docs/connect/custom-accounts">Custom accounts</a>.
 * They can be bank accounts or debit cards as well, and are documented in the links above.
 *
 * Related guide: <a href="https://cloudpayments.com/docs/payments/bank-debits-transfers">Bank debits and transfers</a>
 *
 * @property string $id Unique identifier for the object.
 * @property string $object String representing the object's type. Objects of the same type share the same value.
 * @property null|string|\Voxel\Vendor\CloudPayments\Account $account The ID of the account that the bank account is associated with.
 ...
 * @property string $currency Three-letter <a href="https://cloudpayments.com/docs/payouts">ISO code for the currency</a> paid out to the bank account.
 * @property null|string|\Voxel\Vendor\CloudPayments\Customer $customer The ID of the customer that the bank account is associated with.
 ...
 * @property null|\Voxel\Vendor\CloudPayments\CloudPaymentsObject $future_requirements Information about the <a href="https://cloudpayments.com/docs/connect/custom-accounts/future-requirements">upcoming new requirements for the bank account</a>, including what information needs to be collected, and by when.
 ...
 * @property null|\Voxel\Vendor\CloudPayments\CloudPaymentsObject $requirements Information about the requirements for the bank account, including what information needs to be collected.
 * @property null|string $routing_number The routing transit number for the bank account.
 * @property string $status <p>For bank accounts, possible values are <code>new</code>, <code>validated</code>, <code>verified</code>, <code>verification_failed</code>, or <code>errored</code>. A bank account that hasn't had any activity or validation performed is <code>new</code>. If CloudPayments can determine that the bank account exists, its status will be <code>validated</code>. Note that there often isn’t enough information to know (e.g., for smaller credit unions), and the validation is not always run. If customer bank account verification has succeeded, the bank account status will be <code>verified</code>. If the verification failed for any reason, such as microdeposit failure, the status will be <code>verification_failed</code>. If a payout sent to this bank account fails, we'll set the status to <code>errored</code> and will not continue to send <a href="https://cloudpayments.com/docs/payouts#payout-schedule">scheduled payouts</a> until the bank details are updated.</p><p>For external accounts, possible values are <code>new</code>, <code>errored</code> and <code>verification_failed</code>. If a payouts fails, the status is set to <code>errored</code> and scheduled payouts are stopped until account details are updated. In India, if we can't <a href="https://support.cloudpayments.com/questions/bank-account-ownership-verification">verify the owner of the bank account</a>, we'll set the status to <code>verification_failed</code>. Other validations aren't run against external accounts because they're only used for payouts. This means the other statuses don't apply.</p>
 */
class BankAccount extends ApiResource
{
     ...
     *
     * @see https://cloudpayments.com/docs/api/external_account_bank_accounts/object#account_bank_account_object-status
     */
