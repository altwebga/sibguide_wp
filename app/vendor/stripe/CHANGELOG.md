...
* [#1647](https://github.com/stripe/stripe-php/pull/1647) Fix \Voxel\Vendor\Stripe\Tax\Settings::update
...
* [#1612](https://github.com/stripe/stripe-php/pull/1612) Report usage of .save and StripeClient
  * Reports uses of the deprecated `.save` and of `StripeClient` in `X-Stripe-Client-Telemetry`. (You can disable telemetry via `\Voxel\Vendor\Stripe\Stripe::setEnableTelemetry(false);`, see the [README](https://github.com/stripe/stripe-php/blob/master/README.md#telemetry).)
...
2. **(Alternative option) Specify a version other than `2023-08-16` when initializing `stripe-php`.**
     ...
     - $stripe = new \Voxel\Vendor\Stripe\StripeClient('sk_test_xyz');
...
* [#1241](https://github.com/stripe/stripe-php/pull/1241) Add generic parameter to \Voxel\Vendor\Stripe\Collection usages
...
* [#1155](https://github.com/stripe/stripe-php/pull/1155) Add streaming methods to Service infra
  ...
  * `\Voxel\Vendor\Stripe\RequestOptions::parse` now clones its input if it is already a `RequestOptions` object, to prevent accidental mutation.
...
## 7.25.0 - 2020-02-14
* [#879](https://github.com/stripe/stripe-php/pull/879) Make `\Voxel\Vendor\Stripe\Collection` implement `\Countable`
...
-   Use `\Voxel\Vendor\Stripe\<class>::class` constant instead of strings ([#643](https://github.com/stripe/stripe-php/pull/643))
...
-   [#539](https://github.com/stripe/stripe-php/pull/539) Fix methods on `\Voxel\Vendor\Stripe\PaymentIntent` to properly pass arguments to the API.

## 6.19.4 - 2018-10-11

-   [#534](https://github.com/stripe/stripe-php/pull/534) Fix PSR-4 autoloading for `\Voxel\Vendor\Stripe\FileUpload` class alias

## 6.19.3 - 2018-10-09

-   [#530](https://github.com/stripe/stripe-php/pull/530) Add constants for `flow` (`FLOW_*`), `status` (`STATUS_*`) and `usage` (`USAGE_*`) on `\Voxel\Vendor\Stripe\Source`
...
-   [#520](https://github.com/stripe/stripe-php/pull/520) Rename `\Voxel\Vendor\Stripe\FileUpload` to `\Voxel\Vendor\Stripe\File`
...
-   [#502](https://github.com/stripe/stripe-php/pull/502) Add `isDeleted()` method to `\Voxel\Vendor\Stripe\StripeObject`

## 6.12.0 - 2018-07-28

-   [#501](https://github.com/stripe/stripe-php/pull/501) Add support for scheduled query runs (`\Voxel\Vendor\Stripe\Sigma\ScheduledQueryRun`) for Sigma
...
-   [#498](https://github.com/stripe/stripe-php/pull/498) Internal improvements to the `\Voxel\Vendor\Stripe\ApiResource.classUrl()` method
...
-   [#493](https://github.com/stripe/stripe-php/pull/493) Add PHPDoc for `auto_advance` on `\Voxel\Vendor\Stripe\Invoice`
...
-   [#481](https://github.com/stripe/stripe-php/pull/481) Add new `\Voxel\Vendor\Stripe\Discount` and `\Voxel\Vendor\Stripe\OrderItem` classes, add more PHPDoc describing object attributes
...
-   [#479](https://github.com/stripe/stripe-php/pull/479) Fix unnecessary traits on `\Voxel\Vendor\Stripe\InvoiceLineItem`

## 6.7.2 - 2018-05-28

-   [#471](https://github.com/stripe/stripe-php/pull/471) Add `OBJECT_NAME` constant to all API resource classes, add `\Voxel\Vendor\Stripe\InvoiceLineItem` class
...
-   [#458](https://github.com/stripe/stripe-php/pull/458) Add PHPDoc for `account` on `\Voxel\Vendor\Stripe\Event`
...
-   [#453](https://github.com/stripe/stripe-php/pull/453) Add constants for `reason` (`REASON_*`) and `status` (`STATUS_*`) on `\Voxel\Vendor\Stripe\Dispute`

## 6.3.2 - 2018-02-27

-   [#452](https://github.com/stripe/stripe-php/pull/452) Add PHPDoc for `amount_paid` and `amount_remaining` on `\Voxel\Vendor\Stripe\Invoice`

## 6.3.1 - 2018-02-26

-   [#443](https://github.com/stripe/stripe-php/pull/443) Add event types as constants to `\Voxel\Vendor\Stripe\Event` class
...
-   [#442](https://github.com/stripe/stripe-php/pull/442) Fix PHPDoc for `\Voxel\Vendor\Stripe\Error\SignatureVerification`
...
*   `\Voxel\Vendor\Stripe\AttachedObject` no longer exists. Attributes that used to be instances of `\Voxel\Vendor\Stripe\AttachedObject` (such as `metadata`) are now instances of `\Voxel\Vendor\Stripe\StripeObject`.

-   Attributes that used to be PHP arrays (such as `legal_entity->additional_owners` on `\Voxel\Vendor\Stripe\Account` instances) are now instances of `\Voxel\Vendor\Stripe\StripeObject`, except when they are empty. `\Voxel\Vendor\Stripe\StripeObject` has array semantics so this should not be an issue unless you are actively checking types.

*   `\Voxel\Vendor\Stripe\Collection` now derives from `\Voxel\Vendor\Stripe\StripeObject` rather than from `\Voxel\Vendor\Stripe\ApiResource`.
...
-   Add `\Voxel\Vendor\Stripe\Stripe::setLogger()` to support an external PSR-3 compatible logger
...
-   Rename `\Voxel\Vendor\Stripe\Object` to `\Voxel\Vendor\Stripe\StripeObject` (PHP 7 compatibility)
