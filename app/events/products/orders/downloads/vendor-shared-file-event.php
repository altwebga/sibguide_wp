...
namespace Voxel\Events\Products\Orders\Downloads;

if ( ! defined('ABSPATH') ) {
	exit;
}

class Vendor_Shared_File_Event extends \Voxel\Events\Base_Event {
	...
	public function get_key(): string {
		return sprintf( 'product-types/%s/downloads/new_file_shared_by_vendor', $this->product_type->get_key() );
	}

	public function get_label(): string {
		return 'Downloads: Vendor shared new file';
	}
	...
	public static function notifications(): array {
		return [
			'customer' => [
				...
				'inapp' => [
					'enabled' => false,
					'subject' => 'Order #@order(:id): New downloads available',
				...
				'email' => [
					...
					'subject' => 'Order #@order(:id): New downloads available',
					'message' => <<<HTML
					New downloadable files are available for order #@order(:id).<br>
			...
			'vendor' => [
				...
				'inapp' => [
					'enabled' => false,
					'subject' => 'Order #@order(:id): Files shared with customer',
					'details' => function( $event ) {
						return [
							'order_item_id' => $event->order_item->get_id(),
						];
