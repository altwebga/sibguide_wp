			...
			if ( $plan->get_key() === 'default' ) {
				...
				if ( $membership->get_type() === 'subscription' && $payment_mode === 'subscription' && $membership->is_switchable() ) {
					...
					if ( $payment_mode === 'subscription' ) {
						...
						$trial_allowed = $user->is_eligible_for_free_trial();
						...
						$args = [
							...
							'billing_address_collection' => \Voxel\get( 'settings.membership.checkout.billing_address_collection', 'auto' ) === 'required' ? 'required' : 'auto',
							'customer_update' => [
								'address' => 'auto',
								'name' => 'auto',
							],
						...
						if ( \Voxel\get( 'settings.membership.checkout.phone_number_collection.enabled', false ) ) {
